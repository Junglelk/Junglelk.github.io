...
> 本文首发在个人 Github 主页，做总结式的记录用。  

至少我在 AI 普及前学习时，看一些大佬们写的技术文档，文档大多给我一种很诡异的感觉：懂的人不需要看，不懂的人看不懂。而有很多被视为“你已经具备了”的背景知识，则又被上下文掩盖住了，让不懂的人连入手都困难。  

本文章描述的是多线程编程领域中，“多线程”为什么出现，解决了什么问题，又带来了什么问题，以及问题的解决方案。从最原始的单线程模型推广到任意线程，内容尽可能不深入技术细节以避免上文谈到的那个悖论。

# 程序

在本文的语境下，更多关注的是程序真正执行的部分，简单来说就是**指令**和**数据**。一个具体的例子是，假如我们有一个最简单的计算器程序，他只能执行加法，那么
“相加”就是指令，而数据就是要加起来的两个数。至于减法、乘法和除法，都可以用加法的形式去表达。同样的原理，任意程序反应到 CPU 上执行时，都只是基于**指令**来操作**数据**。

# 内存

内存就是计算机中用于存储**指令**与**数据**的介质，更准确说，是存储要被执行的程序里**指令**和**数据**的介质，它由一系列存储单元组成，每个单元可以存储一个比特（0 或 1）。
[静态随机存储器（SRAM）](https://en.wikipedia.org/wiki/Static_random-access_memory)，是随机存储器的一种。所谓“静态”，是指这种存储器只要保持通电，存储的数据就可以恒常保持。相对之下，[动态随机存储器（DRAM）](https://en.wikipedia.org/wiki/Dynamic_random-access_memory)
里面所存储的数据就需要周期性地更新。然而，当电力供应停止时，SRAM存储的数据还是会消失（被称为易失性存储器），这与在断电后还能存储数据的ROM或闪存是不同的。

本文语境中，内存指 DRAM。

# CPU

**[中央处理器 Central Process Unit](https://en.wikipedia.org/wiki/Central_processing_unit)** 为计算机的主要设备之一，功能主要是解释**指令**以及处理计算机软件的**数据**。
...
```txt
CPU 寄存器（最快）-> L1 Cache -> L2 Cache -> L3 Cache -> 主内存 DRAM（慢） -> SSD/HDD（更慢）
```
...
保证了写完后数据全局可见，但不保证写的是对的，或者说不保证写的结果符合符合程序的**as-if-serial**的语义（因为硬件范畴，并无程序语义）。
...
那结果自然是没有任何问题的，但如果在 Core0 与 Core1 竞争时，总线裁定 Core1 先写，写入顺序就变成了 A、C、B。这对 CPU
来说不会有任何问题，但对程序员来说，这就是出了 Bug。   
这就是我们上文提到的线程安全问题:
...
Buffer，写操作的发起可能会被延迟执行，为了避免这个问题，RMW 采用专门的写通路来实现直写，不经过 Store Buffer，将值直接写入到
L1 缓存，并传播到其他 CPU。  
下面是一些典型的 RMW 指令（x86）
...
CPU 的天然的执行语义。又因为它做的事情仅仅是比较、交换，对于新旧值的定义或程序逻辑毫无定义，给了用户极大的自由：可以借助这一点来实现任意的原子指令。
...
CPU 时间，原本避免线程挂起节省的时间又会因为线程无意义的自旋浪费掉，因此互斥锁依然有它的使用场景，即使它看起来“很重”。
...
*序理论 [Order Theory](https://en.wikipedia.org/wiki/Order_theory)*。序理论研究的对象为“先后”、“大小”这种直觉观念，并通过一套严密的规则来将这些直觉定义出来。
在本文中，我们关注的是**先后**，这种序，那么针对任意两个事物\<A,B>，就必然会存在如下先后关系之一
...
> 至于 A，B 没有关系，是指他们不存在比较关系，至于它们真实发生的时机是否存在先后，是否"同时"，对各自的结果没有影响，因此也就不存在序。

于是我们从这些关系中，抽象出两种描述先后的方式
...
> 内存模型描述了程序的可能行为。内存模型的实现可以自由生成和执行代码，只要其结果可被内存模型推导。
> -- Java Language Specification 21

我们前面提到，程序的本质是指令和数据，Java 程序也是一样，指令和数据由进程管理，存放在内存中，而**线程**是执行指令、修改数据的实体。上面我们谈到线程间存在共享变量时，就可能导致线程安全问题，内存模型很大程度上就是为了解决共享变量的可见性问题。所以必须对共享变量进行一次明确的认知。
...
**针对共享变量的两次访问（读或写），且至少有一次是写**时，这两次访问被认为是**冲突**的。
...
这里的 release 与 acquire 实际上是建立可见性的最小因果关系，它并不局限于 JMM，而是广泛存在于任意场景下的并发设计中，比如C/C++
内存模型，操作系统并发理论的 publish（release）和 consume（acquire）。release 表示 `我做完了一件事，这件事的结果可被看到`
...
如果该执行的 `A` 满足`happens-before consistent`，那么该执行是 **happens-before consistent**的。(An execution is
...
限于篇幅和精力，对硬件原理和编程语言设计做了诸多简化，有兴趣的读者可自行寻找切入点查询详细资料。JMM 的部分绝大多数内容参考 *The Java Language Specification - 21* 的 第17章第四节 *Memory Model*。