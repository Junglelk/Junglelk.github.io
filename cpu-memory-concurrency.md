...
[静态随机存储器（SRAM）](https://en.wikipedia.org/wiki/Static_random-access_memory)
，是随机存储器的一种。所谓“静态”，是指这种存储器只要保持通电，存储的数据就可以恒常保持。相对之下，[动态随机存储器（DRAM）](https://en.wikipedia.org/wiki/Dynamic_random-access_memory)
...
*The Java Language Specification - 21* 的 第17章第四节 *Memory Model*。
